...
class TrainTransform:
    def __init__(self, aug_mode):
        ...
        if self.aug_mode == 1:
            ...
            # Augmentations without random rotation and translation around z-axis
            t = [JitterPoints(sigma=0.05, clip=0.1)]
            # t = [JitterPoints(sigma=0.1, clip=0.2), RemoveRandomPoints(r=(0.0, 0.1)),
            #      RemoveRandomBlock(p=0.4)]            
        else:
...
class TrainRGBTransform:
    def __init__(self, aug_mode):
        ...
        if self.aug_mode == 1:
            ...
            t = [transforms.ToTensor(),
                 transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
                 transforms.RandomErasing(scale=(0.1, 0.3)),
                 transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]
        else:
            raise NotImplementedError('Unknown aug_mode: {}'.format(self.aug_mode))
        self.transform = transforms.Compose(t)
...
class RandomTransformation:
    ...
    def __call__(self, coords):
        ...
        R = np.array([[np.cos(theta), -np.sin(theta), 0],
                      ...
                      [0, 0, 1]])
        t = np.zeros(3)
        t[:2] = self.max_delta * np.random.randn(2)
        # print('translation', t)
        T[:3, :3] = R
        T[:3, 3] = t
        coords = coords @ R.transpose() + t
        return coords, T
