...
class MyVoxelSetAbstraction(nn.Module):
    ...
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, shared_embeddings=False, **kwargs):
        super().__init__()
        ...
        if not shared_embeddings:
            self.vsa_point_feature_fusion2 = nn.Sequential(
                nn.Linear(c_in, self.model_cfg.NUM_OUTPUT_FEATURES, bias=False),
                nn.BatchNorm1d(self.model_cfg.NUM_OUTPUT_FEATURES),
                nn.ReLU(),
            )
    ...
    def forward(self, batch_dict, compute_embeddings=True, compute_rotation=True):
        ...
        if not self.shared_embeddings:
            if compute_rotation:
                point_features_fusion1 = self.vsa_point_feature_fusion(point_features.view(-1, point_features.shape[-1]))
            else:
                point_features_fusion1 = None

            if compute_embeddings:
                point_features_fusion2 = self.vsa_point_feature_fusion2(point_features.view(-1, point_features.shape[-1]))
            else:
                point_features_fusion2 = None

            batch_dict['point_features'] = point_features_fusion1  # (BxN, C)
            batch_dict['point_features_NV'] = point_features_fusion2  # (BxN, C)
        else:
            point_features_fusion1 = self.vsa_point_feature_fusion(point_features.view(-1, point_features.shape[-1]))
            batch_dict['point_features'] = point_features_fusion1  # (BxN, C)
            batch_dict['point_features_NV'] = point_features_fusion1  # (BxN, C)
