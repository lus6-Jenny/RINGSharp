...
from glnet.utils.data_utils.point_clouds import generate_bev, generate_bev_occ, generate_feats, generate_bev_feats
...
class RINGPlusPlus(nn.Module):
    def __init__(self, model_params: ModelParams):
        super(RINGPlusPlus, self).__init__()
    ...
    def forward(self, batch):
        ...
        if self.use_bev:
            ...
            bev = generate_feats(orig_pc, Z=self.Z, Y=self.Y, X=self.X, bounds=self.bounds).cuda()
            # bev = generate_feats(orig_pc, Z=self.Z, Y=self.Y, X=self.X, bounds=self.bounds).unsqueeze(0).cuda()
