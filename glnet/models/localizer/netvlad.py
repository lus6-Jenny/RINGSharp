...
from glnet.models.aggregation.NetVLADLoupe import *
from glnet.utils.spconv_utils import find_all_spconv_keys
import torchvision
...
class NetVLAD(torch.nn.Module):
    def __init__(self, model_params: ModelParams):
        ...
        self.pooling = NetVLADLayer(num_clusters=32, dim=self.image_feat_dim)
    ...
    def extract_image_feature(self, img):
        ...
        return img_feats   # Tensor: (B*S, C, H, W)


    def forward(self, batch):
        pc = batch['pc']    # B,V,4 
        im = batch['img']   # B,S,C,H,W, S = number of cameras
        if pc is not None:
            B, V, _ = pc.shape
        elif im is not None:
            B, S, C, H, W = im.shape
        ...
        #     cv2.imwrite("./img"+ str(i)+ ".png", cv2.cvtColor(image, cv2.COLOR_RGB2BGR))

        x = self.extract_image_feature(im)  # (B*S, C, H, W)
        x = __u(x)
        _, S, C, H, W = x.shape
        x = x.reshape(B, -1, H, W)
        x = x.permute(0,2,3,1)
