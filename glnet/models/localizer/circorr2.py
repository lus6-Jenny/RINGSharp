...
class circorr2(nn.Module):
    ...
    def __init__(self, is_circular=True, zero_mean_normalize=True):
    ...
    def circular_corr(self, x1, x2, scale=1, normalize=True):
        if x1.dim() == 3:
            x1 = x1.unsqueeze(0)
        if x2.dim() == 3:
            x2 = x2.unsqueeze(0)            
        ...
        # if normalize:
        #     corr = F.normalize(corr, p=2, dim=-1)        
    ...
    def curve_smoothing(self, F, S):
        ...
        # zero-pad the curve
        padded_F = torch.concat((F[:,:int(W/2)], torch.zeros(B,(S-1)*W).to(F.device), F[:,int(W/2):]), dim=-1)
